'use client' // Astro ignores this directive — harmless to keep for editor tooling

import { useEffect, useRef, useState } from 'react'

/** Heading entry used by the Table of Contents scroll-spy */
...
/** Glass-styled Table of Contents with scroll-spy (React island, client:load) */
      ...
      <ul className="space-y-0.5">
        {headings.map((heading) => (
          <li key={heading.id} style={{ paddingLeft: `${Math.max(0, heading.level - 2) * 12}px` }}>
            <a
              href={`#${heading.id}`}
              className={`block cursor-pointer rounded-lg px-2.5 py-1.5 transition-all duration-200 ${
                activeId === heading.id
